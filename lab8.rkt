...
(define (convert10 number base)
  (if (not(= number 0)) (append(convert10 (quotient number base) base)(list(modulo number base))) '()))
...
(define (base10 oldbase numlist)
  (base10tail oldbase 1 (reverse numlist)))

(define (base10tail oldbase multiplier numlist)
  (if (null? numlist) 0 (+(*(car numlist) multiplier) (base10tail oldbase (* multiplier oldbase) (cdr numlist)))))
...
(define (geo-prog cur_num ratio n)
  (if (= n 0) '() (append (geo-prog cur_num ratio (- n 1)) (list(* cur_num (expt ratio (- n 1)))))))