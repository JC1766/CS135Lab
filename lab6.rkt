...
(define (power relation k)
  (if (= k 0) '() (if (= k 1) relation (composite relation (power relation (- k 1))))))
...
(define (transitive-helper relation n)
  (if (= n 0) '() (append (power relation n) (transitive-helper relation (- n 1)))))
(define (transitive-closure relation)
  (make-set (append relation (transitive-helper relation (length relation)))))
