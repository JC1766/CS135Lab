...
(define (element? item list-of-items)
  (if (eqv? '() list-of-items) #f (if (eqv? item (car list-of-items)) #t (element? item (cdr list-of-items)))))
...
(define (subset? setA setB)
  (if (null? setA) #t (if (element? (car setA) setB) (subset? (cdr setA) setB) #f)))
...
(define (set-equal? setA setB)
  (if (and (subset? setA setB) (= (length setA) (length setB))) #t #f))
