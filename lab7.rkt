...
(define (degree relation vertex)
  (if (equal? relation '()) 0 (if (and (= (car(car relation)) vertex) (= (car(cdr(car relation))) vertex)) (+ 2 (degree (cdr relation) vertex))
...
(define (vertex-helper relation int)
  (if (null? relation) #t (if (>= int (car(car relation))) (and #t (vertex-helper (cdr relation) int)) #f)))
  
(define (maxVertex relation)
   (if (null? relation) 1 (if (vertex-helper relation (car(car relation))) (car(car relation)) (maxVertex (cdr relation)))))
...
(define (euler relation rel)
   (if (null? rel) #t (if (=(modulo(degree relation (car(car rel)))2)0) (and #t (euler relation (cdr rel))) #f)))
(define (has-eulerian-cycle? relation)
  (euler relation relation))
