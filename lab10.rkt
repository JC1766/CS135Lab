...
(define (M tuplelist)
  (if (null? tuplelist) 1 (* (cadar tuplelist) (M (cdr tuplelist)))))
...
(define (CRT-exists? tuplelist)
  (if (null? tuplelist) #t (and (exist (cadar tuplelist) (cdr tuplelist)) (CRT-exists? (cdr tuplelist)))))
(define (exist b tuplelist)
  (if (null? tuplelist) #t (if (= (EuclidAlgo b (cadar tuplelist)) 1) (and #t (exist b (cdr tuplelist))) #f)))
...
(define (CRT-helper tuplelist m)
  (if (null? tuplelist) 0 (+ (* (caar tuplelist) (* (/ m (cadar tuplelist)) (mul_inv (/ m (cadar tuplelist)) (cadar tuplelist))))
...
(define (CRT tuplelist)
  (CRT1 tuplelist (CRT-helper tuplelist (M tuplelist))))
(define (CRT1 tuplelist int)
  (if (CRT-exists? tuplelist) (if (< int 0)
     (CRT1 tuplelist (+ int (M tuplelist))) (if (> (quotient int (M tuplelist)) 1) (CRT1 tuplelist (- int (M tuplelist))) int)) -1))
