...
(define (id n)
   (if (= n 0) '() (append (id (- n 1)) (list(list n n)))))
...
(define (flip-pairs relation)
    (if (null? relation) '() (append (list(reverse(car relation))) (flip-pairs (cdr relation)))))
...
(define (symmetric-closure relation)
  (if (symmetric? relation) relation (make-set(union relation (flip-pairs relation)))))
...
(define (related-to element relation)
   (if (null? relation) '() (if (= element (car(car relation))) (append (cdr(car relation)) (related-to element (cdr relation)))
